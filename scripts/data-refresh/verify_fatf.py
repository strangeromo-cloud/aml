...
def note_list_date(note: str) -> str | None:
    """Pull the *list* date out of a row-1 note.

    The note can carry more than one date — a sheet generated by
    build_legal_workbook ends with "抓取: 2026-08-17" — so an unlabelled
    first-match grabs the fetch timestamp and the baseline reads as far newer than
    it is. Prefer whatever follows the 名单日期 / list date label, and accept the
    "June 2026" form the FATF page itself uses.
    """
    text = str(note or "")
    labelled = re.search(r"(?:名单日期|名單日期|list\s*date)\s*[:：]?\s*([^·|]{3,40})",
                         text, re.I)
    if labelled:
        d = parse_date(labelled.group(1))
        if d:
            return d
    # No label: only trust the note when it is unambiguous.
    all_iso = re.findall(r"\d{4}-\d{2}-\d{2}", text)
    if len(set(all_iso)) == 1:
        return all_iso[0]
    return None
