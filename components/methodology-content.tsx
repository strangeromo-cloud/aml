...
const FACTOR_CATALOG: Record<keyof typeof DIMENSION_NAMES, FactorMeta[]> = {
  ...
  pepMedia: [
    ...
    },
    {
      ...
      formula: "min(100,  8 × n)",
      meaning: {
        en: "Articles in the last 24 months mentioning the entity in connection with AML/fraud/sanctions/corruption keywords. Each article adds 8 points; capped at 100.",
        zh: "过去 24 个月内涉及反洗钱/欺诈/制裁/腐败关键词的负面新闻数量。每篇文章 +8 分，上限 100。",
      },
      example: {
  ...
  enrichment: [
    ...
    },
    {
      ...
      sourceIds: ["wjp_rol"],
      dataReality: "real",
      formula: "(1 − wjp_score) × 100",
      meaning: {
        en: "WJP Rule of Law Index 2025 overall score (0..1, higher = stronger). Inverted to a 0..100 risk score so 'weaker rule of law → higher risk'. Refreshed daily from public/downloads/wjp-rule-of-law.xlsx; falls back to a WGI-derived proxy for the few jurisdictions WJP doesn't cover.",
        zh: "WJP 法治指数 2025 综合得分（0-1，越高越强），反向映射为 0-100 风险分（法治弱 = 风险高）。每天从 public/downloads/wjp-rule-of-law.xlsx 刷新；WJP 未覆盖的少数国家回退到 WGI 代理。",
      },
      example: {
        en: "Germany WJP 0.833  →  risk 17     Venezuela WJP 0.260  →  risk 74",
        zh: "德国 WJP 0.833 → 风险 17     委内瑞拉 WJP 0.260 → 风险 74",
      },
    },
    {
      ...
      sourceIds: ["tjn_fsi"],
      dataReality: "real",
      formula: "secrecy_score  (0 – 100, higher = more opaque)",
      meaning: {
        en: "Tax Justice Network Financial Secrecy Index 2022 — per-jurisdiction secrecy score on 0..100. Refreshed daily from public/downloads/tjn-fsi.xlsx; falls back to a hand-curated estimate for the few jurisdictions TJN doesn't rank.",
        zh: "Tax Justice Network 金融保密指数 2022 —— 各辖区保密评分 0-100。每天从 public/downloads/tjn-fsi.xlsx 刷新；TJN 未排名的少数辖区回退到人工估值。",
      },
      example: {
        en: "United States 68.6   ·   Switzerland 74.9   ·   Finland 39.6",
        zh: "美国 68.6   ·   瑞士 74.9   ·   芬兰 39.6",
      },
    },
  ],
};
